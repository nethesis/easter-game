...
const initFiles = async () => {
  try {
    await s3.send(
      new HeadObjectCommand({ Bucket: BUCKET_NAME, Key: ACTIVE_PLAYERS_FOLDER })
    );
    console.log(`Active players folder exists: ${ACTIVE_PLAYERS_FOLDER}`);
  } catch (error) {
    if (error.name === "NotFound") {
      console.log(
        `Active players folder does not exist. Creating: ${ACTIVE_PLAYERS_FOLDER}`
      );
  ...
  try {
    const data = await s3.send(
      new GetObjectCommand({ Bucket: BUCKET_NAME, Key: PLAYERS_JSONL_FILE })
    );
...
const readPlayerFile = async (fileKey) => {
  try {
    ...
    console.error(
      `Error checking existence of player file (${fileKey}):`,
      error
    );
    return null;
  }
